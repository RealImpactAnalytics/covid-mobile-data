...
class DataSource:
    ...
    # constructor
    def __init__(self, spark, config_input):
    ...
    #     else:
    #         raise Exception(
    #             'Spark session not initialised. Specify type (local, cluster, hive) of spark connection in config file, and/or modify the setup_spark.py file.')
    ...
    def setup_config(self, input_config):
        ...
        for config_key in input_config:
            ...
            if config_key not in keys_types_defaults:
                raise Exception('Key input_config["' + config_key + '"] is not an allowed key. Allowed keys are: ',
                                keys_types_defaults.keys())

            # test that values that are of correct type
            if not isinstance(input_config[config_key], keys_types_defaults[config_key][0]):
                raise Exception('Input input_config["' + config_key + '"] is not of type ' + str(
                    keys_types_defaults[config_key][0]))
    ...
    def show_config(self):
        ...
        print("Load options:", {"seperator": self.load_seperator, "header": self.load_header, "mode": self.load_mode,
                                "datemask": self.load_datemask})
    ...
    def load_parquet_file_with_vars(self, region):
        self.parquet_vars_df = self.spark.read.format("parquet").load(
            self.standardize_path + "/" + self.parquetfile_vars + region)
    ...
    def sample(self, number_of_ids=10000, seed_to_use=510, since_date=dt.datetime(2020, 2, 2)):
        ...
        self.unique_ids = [i.msisdn for i in self.parquet_df.where(F.col('call_datetime') < since_date).select(
            'msisdn').distinct().collect()]
